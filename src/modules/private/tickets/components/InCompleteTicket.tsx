...
import { methodDeleteService } from "../../../../shared/utils/api-helper";
import type { ICompleteTicket } from "../../../../shared/utils/interface.util";
import { FaRegCircleQuestion } from "react-icons/fa6";

const InCompleteTicket = (props: ICompleteTicket) => {
	const { isShowModal, ticketId, titleTicket, assigneeName,  setIsShowModal = () => { }, onFinish = () => { } } = props;
	const [isLoading, setIsLoading] = useState<boolean>(false);
	const [messageApi, contextHolder] = message.useMessage();

	const onInCompleteTicket = () => {
		const url = `${GET_LIST_TICKET}/${ticketId}/complete`;
		return from(methodDeleteService(url))
	}
	const onSubmit = () => {
		setIsLoading(true);
		onInCompleteTicket().pipe(
			catchError((error) => {
	...
	return <div>
		...
		>
			<div className="flex gap-1">
				<p>
					<span>
						Do you want to incomplete ticket:
					</span>
...
export default InCompleteTicket;